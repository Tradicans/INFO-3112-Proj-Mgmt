...
const TeamMemberComponent = (props) => {
  ...
  const readTeamArray = async () => {
    ...
    let query = `query {usersbyproduct(productid: "${state.selectedProduct._id}"){name, role}}`;
    let json = await queryFunction(query);
    //todo: check this returns just the team array of names as expected
    setState({ teamArray: json.data.usersbyproduct });
  ...
  const onAddClicked = async () => {
    ...
    let query = `mutation{adduser(name:"${state.newName}",role:"${state.newRole}",products:"${state.selectedProduct._id}"){_id, name, role}}`;
    let json = await queryFunction(query).then(
      async () => await readTeamArray()
    );
    if (json !== null) {
